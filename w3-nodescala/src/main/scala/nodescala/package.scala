...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      val empty: Future[List[T]] = Future(Nil)
      fs.foldLeft(empty)((acc, f) => acc flatMap (vacc => f map (_ :: vacc))) map (_.reverse)
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      fs foreach (_ map promise.success recover {case e => promise failure e})
      promise.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      f map (_ => cont(f)) recoverWith { case e => Future(throw e) }
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      ...
      f map { value =>
          promise complete Try(cont(Success(value)))
        } recover { case _ => promise failure _ }
      promise.future
    }
